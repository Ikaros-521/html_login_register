...
int cgiMain(void)
{
	...
	if (SQLITE_OK == result)
	{
		for (; i < nRow; i++)
		{
			if (strncmp(username, dbResult[nColumn + 2 * i], 20) == 0 && strncmp(password, dbResult[nColumn + 2 * i + 1], 40) == 0)
			{
				printf("<script>alert(\"登录成功！\")</script>\n\n");
				break;
			}
			else if (i == nRow - 1)
			{
				printf("<script>alert(\"帐号或密码错误\");</script>\n\n");
